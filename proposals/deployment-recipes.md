...
> **Blocked on a runtime-version limitation (own PR, not merged with `solo`).** The build-time ingest of the richly-referential brain-bank data hits `FOREIGN KEY constraint failed` on the currently-certified **hippo 0.10.6**, which enforces SQLite foreign keys immediately during ingest; the brain-bank data has cyclic/forward references, so recipe-side bundle ordering can't resolve it (a cycle has no valid insertion order). The identical bundle ingests cleanly on Mosaic 0.11 (deferred-FK / insert-then-check), so this needs the certified frontier to advance to a runtime with deferred-FK ingest — or a decision to degrade the dataset. Tracked in its own follow-up PR; the design below stands.
