...
| 1.8 | Modeler inclusion | ⛔ **SUPERSEDED 2026-08-24 by [platform ADR-0005](../platform/design/decisions/ADR-0005-drop-linkml-modeler-from-recipes.md): the Modeler ships in NO recipe.** It is wired into nothing, is the only from-source build in a production recipe (an exception to 1.4), and was the last pin on EOL Node 20. Mode R survives; the `cors-proxy` does not. The original decision is preserved below for the record. ~~✅ Decided (owner, 2026-07-14): Modeler ships in `solo` by default~~ — users iterating their own schema is a first-class platform capability, not a dev-only feature. CORS proxy included alongside it (needed for the remote workflow, §2.1a). | Since the Modeler can't reach the server's schema dir (§0 fact 2), including it obligates the recipe to close the schema loop in *both* deployment modes — see the workflow analysis in §2.1a. |
| 1.9 | `ide` shape | **Compose file, not single container** — services: `mosaic` (from source checkout *or* pinned image; `--reload` optional), `aperture` (published image), `modeler` + `cors-proxy` (profile `modeler`), everything bind-mounted from the project dir. Plus a `make migrate` / `docker compose restart mosaic` documented inner loop. | Dev wants independent restarts, logs, and source-mounting; cramming that into one container fights compose for no benefit. The IDE recipe is *allowed* to build from source (gate does not apply — ADR-0001 governs deployment, not development). **⚠️ Amended 2026-08-21 by [platform ADR-0004](../platform/design/decisions/ADR-0004-ide-recipe-live-development-loop.md):** `aperture` as a *published image* covers schema iteration but leaves front-end iteration to a rebuild loop. ADR-0004 adds a **`dev-aperture` profile** — the gateway proxies `/` to a Vite dev server (HMR, websocket proxied) — symmetric with `dev-mosaic`. The published image remains the default when no profile is selected; the rest of this row stands. |
...
> **⛔ Superseded by [platform ADR-0005](../platform/design/decisions/ADR-0005-drop-linkml-modeler-from-recipes.md) (2026-08-24).** The Modeler ships in no recipe. **Mode L** survives as plain "edit the bind-mounted `schemas/` directory" — it never needed server-side support, since the Modeler always read the *host* filesystem. **Mode R** survives as pull-on-boot from `SCHEMA_GIT_REMOTE`, but without the bundled `cors-proxy`: users on that path need a CORS-enabled git host or their own proxy. The analysis below is kept for the reasoning, not as a description of the shipped recipe.
...
> **⚠️ Amended by [platform ADR-0004](../platform/design/decisions/ADR-0004-ide-recipe-live-development-loop.md) (2026-08-21).** The inner loop below is *schema* iteration, and the service table lists `aperture` as a published image — so as originally written this section does not give front-end iteration a live loop. ADR-0004 adds a `dev-aperture` compose profile (gateway → Vite dev server, HMR websocket proxied) so a change to **either** component is visible without a rebuild, and requires the recipe's CI boot test to assert the websocket rather than just HTTP 200. Read this section together with that ADR.
...
| ~~`modeler` + `cors-proxy`~~ | ⛔ **removed by [ADR-0005](../platform/design/decisions/ADR-0005-drop-linkml-modeler-from-recipes.md)** — never implemented; datahelix#84 closed as superseded | — |
...
### 2.3a `deploy/recipes/demo/` — populated showcase (follow-up PR)

`solo` runtime with a **large synthetic brain-bank dataset baked into the image**, so it boots straight into a populated Aperture explorer with zero setup. Added after `solo` at the owner's request; it reuses `solo`'s same-origin runtime (nginx + `mosaic serve`, relative `/graphql`) but drops the Modeler (the dataset is immutable — nothing to edit) and the migrate/pull loop.

> **Unblocked as of the frontier advance to Mosaic 0.11.0** (certified via [#59](https://github.com/BU-Neuromics/datahelix/pull/59)). The build-time ingest of the richly-referential brain-bank data previously hit `FOREIGN KEY constraint failed` on the certified **hippo 0.10.6**, which enforced SQLite foreign keys immediately during ingest; the brain-bank data has cyclic/forward references (`Container.parent`, `Location.parent_location`, `AnatomicalStructure.part_of`), so recipe-side bundle ordering could not resolve it (a cycle has no valid insertion order). **Mosaic 0.11.0 defers foreign-key checks to commit** ([mosaic #95](https://github.com/BU-Neuromics/mosaic/issues/95)), so the whole bundle ingests atomically regardless of order; the demo now pins the 0.11.0 image and ingests natively with `mosaic ingest --db-path`.

The dataset is generated **at build time, inside the pinned runtime image** (so the SQLite DB is built by the exact version that serves it — no skew): a data stage `pip install`s [`linkml-data-gen`](https://github.com/BU-Neuromics/linkml-data-gen) (pinned) and clones [`brainbank-hippo-schema`](https://github.com/VA-NCPTSDBB-Bioinformatics/brainbank-hippo-schema) (pinned), generates a referentially-consistent dataset from the schema's tree-root, remaps the bundle's top-level keys to Mosaic's ingest accessors (schema-driven, via the image's own `linkml_bridge`), and ingests it. Scale/seed are build args (`DEMO_COUNTS`, `DEMO_SEED`); generation is deterministic per seed. Certification posture matches `solo` (built from the certified pair; `check-pins`; `gate`); the generator and schema are pinned separately as build inputs. The `demo-recipe.yml` smoke job builds a small dataset and asserts real rows are served through the seam — a populated demo that serves zero rows is a failed demo.

Two integration findings surfaced building this and were fixed at the source rather than papered over in the recipe: (1) `linkml-data-gen` minted per-class ids, so sibling classes sharing an abbreviation (`ATACSeqDataset`/`ATACSeqAssay` → `ATAC`) collided under Mosaic's global entity registry — fixed upstream ([linkml-data-gen#3](https://github.com/BU-Neuromics/linkml-data-gen/pull/3)); (2) Mosaic derives ingest-bundle accessors from class names, diverging from the schema's own tree-root slot names for irregular plurals (`processes`→`processs`) — handled by the recipe's schema-driven remap, which reuses Mosaic's own accessor helper so it self-adapts to the runtime version.
