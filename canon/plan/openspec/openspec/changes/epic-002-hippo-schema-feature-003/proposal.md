...
## Acceptance Criteria
- Given the schema.yaml file exists, when it is parsed as valid YAML, then a top-level entity type named GenomeBuild is present
- Given the GenomeBuild entity type is defined in schema.yaml, when its fields are inspected, then a field named genome_assembly exists with type string
