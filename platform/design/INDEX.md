...
| `roadmap-1.0.md` | **DataHelix 1.0 roadmap** — six-month design-sprint output (2026-07-07): four phases to a certified 1.0 across hippo/aperture/DataHelix. Ratified scope: Bridge minimal in 1.0 (multi-user), portal-complete + MCP agent surface, one Cappella reference adapter. Epic tables with acceptance criteria for agent handoff; supersedes `FABLE_HANDOFF.md` §6 milestones where they conflict. | 🟠 Working |
...
| Mosaic external adapter stubs (STARLIMS, HALO, Donor DB) | To be removed from Mosaic in a future spec update | Currently stubbed in `hippo/adapters/external/`. These implementations should migrate to Cappella. The `ExternalSourceAdapter` ABC remains in Mosaic. |
...
|---|---|---|
| `hippo/design/sec2_architecture.md` | Add `WriteValidator` ABC and `mosaic.write_validators` entry point group to adapter pattern section; add `WriteOperation` and `ValidationResult` to package structure | Validation architecture decision |
...
| `hippo/design/sec2_architecture.md` | Add `ReferenceLoader` ABC and `mosaic.reference_loaders` entry point group to plugin system section | Reference loader plugin system decision |
| `hippo/design/sec2_architecture.md` | Remove `ExternalSourceAdapter` concrete stubs (STARLIMS, HALO, Donor DB) from Mosaic package structure — ABC stays, implementations move to Cappella | Adapter boundary decision |
...
| `hippo/design/sec5_ingestion.md` | *(not started)* Include `mosaic reference install/update/list` CLI commands; `ReferenceLoader` lifecycle; reference loader version tracking in `hippo_meta` | Reference loader plugin system decision |
...
| `hippo/design/INDEX.md` | Add decisions from today's session to Mosaic key decisions log | All Mosaic-touching decisions |
