...
**Depends on:** sec2_components.md (§2.6 Bridge — auth gateway), sec3_integration.md (§3.6 Bridge ↔ all components; auth middleware contract), `domain-graph.md` (knowledge-subgraph semantics), Mosaic `sec8_auth_integration.md`, Bridge `sec4_auth.md`
**Feeds into:** Bridge `sec4_auth.md` (PDP engine), Mosaic `sec4_api_layer.md` (filter pushdown), Aperture ADR-0007/0008/0011/0014/0016/0017/0018/0020 (capability-scoped client, deferred Bridge, BFF, provenance)
...
2. **Mosaic (formerly Hippo, ADR-0004) is auth-unaware** (Mosaic `sec8_auth_integration.md`). In SDK mode `MosaicClient` accepts
   an `actor` string with no validation; in REST + Bridge mode Mosaic reads a *validated* identity
   from Bridge-injected headers and trusts it unconditionally. Mosaic holds no signing keys, user
...
Authorization never lives in Aperture or Mosaic. The "thin BFF in front of Mosaic" that a
...
**injected, capability-scoped client** (Aperture ADR-0008). It is a `MosaicClient`-shaped interface
...
Aperture talks the **same autogenerated GraphQL contract** whether it points at Mosaic directly
...
Predicate pushdown depends on Mosaic's filter expressiveness (notably set-membership / `IN`
filters — see §6.8).
...
- **Mosaic `IN`-filter dependency** — record-level predicate pushdown needs set-membership filters
  in Mosaic's query/GraphQL surface (today: equality + AND/OR + offset + FTS). Same capability-gap
