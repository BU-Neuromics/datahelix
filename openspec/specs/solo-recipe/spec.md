# solo-recipe Specification

## Purpose
`deploy/recipes/solo/` — the single-container Aperture+Mosaic+Modeler MVP
deployment recipe. This capability spec tracks correctness requirements for
loading a Mosaic project (schema + database) via `PROJECT_DIR`, independent
of the certification/composition concerns owned elsewhere in the platform.
